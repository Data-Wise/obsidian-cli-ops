...
## TUI Improvements (v2.2.0-final)

The Terminal User Interface has received a significant polish pass focusing on usability, accessibility, and stability.

### ADHD-Friendly Dashboard
- **Vault-First Workflow**: The app now opens directly to the Vault Browser, eliminating decision fatigue.
- **Action Grid**: The new Dashboard features a clear 2x3 grid for primary actions (Notes, Graph, Stats, Switch Vault, Logs, Help).
- **Status Panel**: Always shows the currently active vault.

### Robust Error Handling
- **Crash Prevention**: Critical errors (like database corruption or permission issues) are now caught gracefully.
- **Error Screen**: A dedicated screen displays error details and tracebacks.
- **One-Click Copy**: Easily copy error details or logs to the clipboard for debugging.
- **Log Viewer**: New `Logs (l)` screen to view and clear application logs in real-time.
...
### Technical Fixes
- **Pagination**: Fixed SQLite syntax errors with LIMIT/OFFSET.
- **Graph Metrics**: Resolved missing data issues in metric retrieval.
- **Logging**: Implemented rotating file logs (`~/.config/obs/obs.log`).
- **Search API**: Added backend support for searching notes by content, vault, and tags with context snippets.
