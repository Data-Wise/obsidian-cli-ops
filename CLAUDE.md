...
- **Pytest**: Testing harness (307 unit + 90 MCP unit + 32 E2E = 429 pytest tests)
...
# Development
pytest src/python/tests/        # Run Python unit tests (307 tests)
pytest tests/test_mcp_server.py # Run MCP unit tests (90 tests)
...
```bash
pytest src/python/tests/        # 307 unit tests passing
pytest tests/test_mcp_server.py # 90 MCP unit tests
...
- `src/python/` - Python backend
  ...
  - `tests/` - Test suite (307 unit + 90 MCP unit + 32 E2E pytest tests)
