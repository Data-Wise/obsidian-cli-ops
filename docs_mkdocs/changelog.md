...
- **Vault templates (`obs template list|new`, MCP `list_templates` + `create_from_template`)** — ports nexus-cli's `vault template` (spec vault-ops row, FS-direct per D3). The templates folder comes from Obsidian's core Templates plugin (`.obsidian/templates.json`), else the obs config `vault.templates` when it belongs to that vault, else `_SYSTEM/templates`, `templates/`, `Templates/`, `_templates/`; a `tpl-` filename prefix is optional. Rendering fills Obsidian's core variables (`{{title}}`, `{{date}}`, `{{time}}`, `{{date:FORMAT}}`) plus `--var KEY=VALUE`; bare `{{date}}`/`{{time}}` honor the vault's Obsidian `dateFormat`/`timeFormat`. Unknown placeholders and Templater `<% %>` are left untouched. A destination outside the vault, a template name outside the templates folder, and an existing note are all refused. MCP tools 42 → 44.
...
- **Docs site sync (#119)** — page version headers were stuck at 4.3.0; the refcard and reference index gain `obs template` entries.
...
- **Homebrew tap: `post_install_steps` migration** — the `obsidian-cli-ops` formula's deprecated `post_install` hook is migrated to `post_install_steps` (`data-wise/homebrew-tap` PR #225); the tap also now bundles `scripts/configure_mcp.py` (previously missing from the installed package entirely) and its caveats point at the real `claude mcp add`/`claude mcp list` commands.
