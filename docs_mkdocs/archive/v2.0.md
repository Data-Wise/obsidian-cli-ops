...
Analyze vault knowledge graph and calculate metrics. Accepts vault name or ID prefix.

```bash
# Analyze by name
obs analyze Knowledge_Base -v

# Or by ID prefix (from obs vaults output)
obs analyze a812 -v
```
...
**Output:**
```
🔍 Searching for vaults in: /Users/dt/Documents
...
**Look for:**
- High orphan count → Need more linking
- Many broken links → Clean up or update links
- Low density → Isolated clusters
